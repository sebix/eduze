...
abstract class Captcha_Driver {
    ...
    public function image_create($background = NULL)
    {
        ...
        if ( ! function_exists('imagegd2'))
            throw new Kohana_Exception('Captcha requires GD2');
    ...
    public function image_render($html)
    {
        // Output html element
        if ($html)
            return '<img alt="Captcha" src="'.url::site('captcha/'.Captcha::$config['group']).'" style="width:'.Captcha::$config['width'].'px;height:'.Captcha::$config['height'].'px" />';
